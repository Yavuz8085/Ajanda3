...
return ( <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1rem", padding: "1rem", fontFamily: "sans-serif", maxWidth: "1000px", margin: "0 auto" }}> <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}> {days.slice(0, 3).map(day => ( <div key={day.key}> <h3>{day.label}</h3> <textarea style={{ width: "100%", height: "80px", fontSize: "1rem" }} value={notes[day.key] || ""} onChange={(e) => handleChange(day.key, e.target.value)} placeholder={Write notes for ${day.label}...} /> </div> ))} </div>
